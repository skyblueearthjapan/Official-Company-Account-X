...
# Compose prompt.md
{
  ...
  echo "  2. Panel 1: below the banner."
  echo "  3. Panel 2: below Panel 1."
  echo "  4. Panel 3: below Panel 2."
  echo "  5. Panel 4: at the bottom."
  echo ""
  echo "Panels are clearly separated with thin black borders. Read top-to-bottom."
  echo ""
  echo "[MANDATORY DIALOGUE LABELING — every speech bubble must follow this rule]"
  echo "Each speech bubble (in every panel) MUST have a small horizontal SPEAKER NAME LABEL BOX positioned IMMEDIATELY ABOVE the bubble (touching or slightly overlapping its top edge). This is a series-wide visual convention."
  echo "  - Label box style: rounded rectangle with DARK NAVY-BLUE / INDIGO fill (#1c2c5a-ish), white text inside, small horizontal pill/tag shape (about 30-50% the width of the bubble)."
  echo "  - Label content: just the speaker's display name in Japanese, e.g., 'I課長', 'セバスチャン', '司会者', '作業員A'"
  echo "  - Every speaker who has a line gets a name label above their bubble. No exceptions."
  echo ""
  echo "[PROHIBITED ELEMENTS — never draw these]"
  echo "  - DO NOT draw '起' '承' '転' '結' markers, labels, or stamps in any corner of any panel"
  echo "  - DO NOT add any kishōtenketsu (起承転結) annotations to the image"
  echo "  - DO NOT add panel-position indicator text (Panel 1, Panel 2, etc.)"
  echo "  - DO NOT add scene labels in panel corners"
  echo ""
  echo "[STYLE]"
  cat "$STYLE_FRAGMENT"
  echo ""
