...
### D. ユーザーが「X に投稿して」と頼んできた場合（自然文での投稿依頼）

「Xに投稿して」「ポストして」「公開して」「ツイートして」「アップして」などの自然文で
**X (旧 Twitter) への投稿依頼** が来たら、以下の手順で対応する。

#### D-1. 対象エピソードを特定する

ユーザーの言及から `episode_id` を解決：

| ユーザーの言い方 | 解決方法 |
|---|---|
| 「001番」「第1弾」「001」「ep001」 | `ls episodes/ \| grep -E '^001-'` で前方一致 |
| 「新拠点」「式典」「45周年」など主題ワード | `ls episodes/ \| grep <キーワード>` で部分一致 |
| 「最新の」「直近の」「次回分」 | `ls episodes/ \| grep -E '^[0-9]{3}-' \| sort -V \| tail -1` |
| 単独依頼で曖昧 | 候補を列挙して聞き返す（**勝手に決め打ちしない**） |

候補が2つ以上ある時は **必ず確認**。1つに絞れた場合のみ次へ。

#### D-2. 投稿準備の前提を確認する

- `episodes/<id>/final/final.png` が存在するか
- `episodes/<id>/final/post-body.txt` が存在し、≤280字 か
- 不在なら欠けている方を起草して提示・承認を得る（この時点ではまだ投稿しない）

#### D-3. 応答末尾に **必ず単独行で** マーカーを出力する

最後の行（または明確に分離された行）に以下のフォーマットで出力：
...
本文（170字、280字以内）と画像を Discord に提示しますので、ボタンで承認してください。

!publish 001-2026-05-11-新拠点完成PR1
```

このマーカーは Discord Bot (`scripts/discord_bot.py`) が後処理でスキャンし、
**承認ボタン付きプレビュー** を自動表示する。スキップやコードブロック内に書いてはならない。

#### D-4. 絶対禁止事項

- ❌ **`scripts/post_to_x.py` を直接実行してはいけない**（curl, tweepy 直叩きも禁止）
- ❌ X API キーを `.env` から読み出して自分で投稿してはいけない
- ❌ ユーザー承認を経ずに投稿してはいけない

承認は必ず Discord 上のボタンで人間が押す。Claude の役割は「投稿対象の特定」と「`!publish <id>` マーカーの出力」までで、実投稿は Bot のフローに委ねる。

#### D-5. 投稿後の振り返り依頼

「投稿の反応どう？」「アナリティクス見たい？」と聞かれたら：
- `analytics/<YYYY-MM>/post-log.md` を読んで投稿履歴を確認
- 実 X API でのメトリクス取得は別途実装予定（Phase 3 後半）。現状は post-log.md ベースで回答
...
- `episodes/001-2026-05-11-新拠点完成PR1/` — 第1弾、Pattern-A v5 採用、final.png + post-body.txt 配置済（投稿可能）
- `episodes/002-2026-05-18-創立45周年記念式典PR2/` — 第2弾、Pattern-C v11 採用、final.png + post-body.txt 配置済（投稿可能）
